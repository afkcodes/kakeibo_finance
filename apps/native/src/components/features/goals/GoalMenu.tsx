/**
 * @fileoverview GoalMenu - Contribute/edit/delete dropdown menu
 * @module @kakeibo/native/components/features/goals
 */

import type { Goal } from '@kakeibo/core';
import { MoreVertical, Pencil, Trash2, Wallet } from 'lucide-react-native';
...
export const GoalMenu = ({ goal, onContribute, onEdit, onDelete }: GoalMenuProps) => {
        ...
        <View className="absolute right-3 top-14 bg-surface-800 border border-surface-700 rounded-xl shadow-lg z-10 min-w-35 overflow-hidden">
          {isActive && onContribute && (
            <>
              ...
              >
                <Wallet size={16} color="#cbd5e1" />
                <Text className="text-surface-200 text-[13px]">Add Money</Text>
              </Pressable>
          ...
          {onEdit && (
            <>
              ...
              >
                ...
                <Text className="text-surface-200 text-[13px]">Edit Goal</Text>
              </Pressable>
              <View className="h-px bg-surface-700" />
            </>
          )}
